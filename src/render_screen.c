...
void draw_image_column(t_game *game, t_ray ray, int column)
{
	...
	while (y < HEIGHT - offset) // THIS DRAWS THE textures.frame
	{
		...
		game->textures.frame.pixels[pixel + 1] = ray.texture.pixels[(current_texture_y * ray.texture.line_size + texture_offset_x) + 1];	// green
		game->textures.frame.pixels[pixel + 2] = ray.texture.pixels[(current_texture_y * ray.texture.line_size + texture_offset_x) + 2];	// red
		game->textures.frame.pixels[pixel + 3] = ray.texture.pixels[(current_texture_y * ray.texture.line_size + texture_offset_x) + 3];	// alpha
		texture_y += 1 * step_y;
		y++;
	}
...
void render_screen(t_game *game)
{
	...
	mlx_put_image_to_window(game->mlx, game->win, game->textures.frame.ptr, 0, 0);
	mlx_put_image_to_window(game->mlx, game->win, game->textures.player.ptr, game->player.x, game->player.y);
	draw_minimap(game);
	draw_direction(game);
}
